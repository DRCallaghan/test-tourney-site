...
import { nextAuthOptions } from '../api/auth/[...nextauth]/options.js'
import { getServerSession } from 'next-auth';
// this is the user team page ONLY. found at /myteam
export default async function Myteam() {
    const session = await getServerSession(nextAuthOptions);
    let userID;
    if (session) {
        userID = session.user.image.substring(17, session.user.image.indexOf("?"));
    };
    return (
